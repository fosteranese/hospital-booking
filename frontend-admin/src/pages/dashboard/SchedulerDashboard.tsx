          ...
          <div className="flex gap-2">
            <div className="flex rounded-lg border border-slate-200 overflow-hidden">
              <button
                ...
                className={`px-3 py-1.5 text-xs font-medium transition-colors ${view === 'today' ? 'bg-emerald-600 text-white' : 'bg-white text-slate-600 hover:bg-slate-50'}`}
              >
                Today
              </button>
              <button
                onClick={() => setView('all')}
                className={`px-3 py-1.5 text-xs font-medium transition-colors ${view === 'all' ? 'bg-emerald-600 text-white' : 'bg-white text-slate-600 hover:bg-slate-50'}`}
              >
                All
              </button>
            </div>
      ...
      {view === 'all' && (
        <Card>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select value={doctorFilter} onChange={(e) => setDoctorFilter(e.target.value)} className={`${inputClass} w-[200px]`}>
              <option value="">All doctors</option>
              {doctors.map((d) => (
                <option key={d.id} value={d.id}>Dr. {d.first_name} {d.last_name}</option>
              ))}
            </select>
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className={`${inputClass} w-[160px]`}>
              <option value="">All statuses</option>
              <option value="confirmed">Confirmed</option>
              <option value="cancelled">Cancelled</option>
            </select>
            <input type="date" value={dateFilter} onChange={(e) => setDateFilter(e.target.value)} className={`${inputClass} w-[170px]`} />
            {hasFilters && (
              <button onClick={() => { setDoctorFilter(''); setStatusFilter(''); setDateFilter(''); }} className="text-sm text-slate-500 hover:text-slate-700 transition-colors">
                Clear all
              </button>
            )}
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-50">
                {appointments.map((a) => (
                  <tr key={a.id} onClick={() => setSelectedAppointmentId(a.id)} className="group cursor-pointer hover:bg-slate-50/50 transition-colors">
                    <td className="px-5 py-3.5 text-sm text-slate-900 font-medium">{a.slot_date}</td>
                    <td className="px-5 py-3.5 text-sm text-slate-600">{formatTime(a.start_time)} – {formatTime(a.end_time)}</td>
