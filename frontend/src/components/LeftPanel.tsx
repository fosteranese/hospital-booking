  ...
  if (status === 'completed') {
    return (
      <div className="size-10 rounded-full bg-white/15 flex items-center justify-center shrink-0">
        <HugeiconsIcon icon={CheckmarkCircle02Icon} strokeWidth={2} className="size-4 text-white/60" />
      </div>
    );
  }
  if (status === 'current') {
    return (
      <div className="size-12 rounded-full bg-primary flex items-center justify-center shadow-lg shadow-primary/20 shrink-0">
        <HugeiconsIcon icon={Icon} strokeWidth={2} className="size-6 text-white" />
      </div>
    );
  }
  return (
    <div className="size-10 rounded-full border-2 border-white/20 flex items-center justify-center shrink-0">
      <HugeiconsIcon icon={Icon} strokeWidth={2} className="size-4 text-white/35" />
    </div>
  );
}
...
export function LeftPanel({ step, wide = false }: LeftPanelProps) {
  ...
  return (
    <aside className={cn("hidden xl:flex relative h-screen overflow-hidden transition-all duration-500 ease-in-out", wide ? "w-[40%]" : "w-[30%]")}>
      <img
                      ...
                      <div key={s.label} className="flex items-start gap-5">
                        <div className="flex flex-col items-center">
                          ...
                          {i < displaySteps.length - 1 && (
                              <div className={cn(
                                'w-px h-12',
                              i < currentIdx ? 'bg-white/15' : 'bg-white/[0.06]',
                            )} />
                          )}
                        </div>
                        <div className={cn(
                          ...
                          {status === 'current' && s.subtitle && (
                            <p className="text-sm text-white/50 mt-0.5">{s.subtitle}</p>
                          )}
                        </div>
                      </div>
                    );
