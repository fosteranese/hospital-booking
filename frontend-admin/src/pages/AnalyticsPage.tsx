...
export function AnalyticsPage() {
  ...
  const { data: combined, loading, error } = useCachedData(
    'analytics',
    useCallback(async () => {
      ...
      return { overview: ov, doctorStats: stats };
    }, [token]),
    { enabled: !!token, staleTime: 120_000 }
  );

  const overview = combined?.overview ?? null;
  const doctorStats = combined?.doctorStats ?? [];
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-right text-[11px] font-semibold text-muted-foreground uppercase tracking-wider px-5 py-3">Attended</th>
                  <th className="text-right text-[11px] font-semibold text-muted-foreground uppercase tracking-wider px-5 py-3">Missed</th>
                  <th className="text-right text-[11px] font-semibold text-muted-foreground uppercase tracking-wider px-5 py-3">Cancelled</th>
                  <th className="text-right text-[11px] font-semibold text-muted-foreground uppercase tracking-wider px-5 py-3">Upcoming</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {doctorStats.map((d) => (
                  <tr key={d.doctor_id} className="transition-all duration-150 hover:bg-muted/80 hover:scale-[1.02] hover:shadow-md" style={{ transformOrigin: 'center' }}>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-sm font-medium text-foreground">Dr. {d.doctor_name}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3.5 text-sm text-muted-foreground">{d.specialization}</td>
                    <td className="px-5 py-3.5 text-sm text-foreground text-right font-medium">{d.total_appointments}</td>
                    ...
                    <td className="px-5 py-3.5 text-sm text-muted-foreground text-right">{d.cancelled}</td>
                    <td className="px-5 py-3.5 text-sm text-foreground text-right font-medium">{d.upcoming}</td>
                  </tr>
                ))}
              </tbody>
            </table>
